...
void CBitmapa::Zobraz_Dlazdici(HDC hWinDC, int x, int y, int sizex, int sizey, int sloupec, int radek, DWORD transparentcolor) {

	if (transparentcolor == 0) transparentcolor = GetPixel(hLocalDC, 0, 0); // if 0 use leftcorner color as transparent
	//BOOL qRetBlit = TransparentBlt(hWinDC, x, y, sizex, sizey, hLocalDC, 0, 0, sirkaDL, vyskaDL, transparentcolor);
	BOOL qRetBlit = TransparentBlt(hWinDC, x, y, sizex, sizey, hLocalDC, 32*sloupec, 32*radek, 32, 32, transparentcolor);
	if (!qRetBlit) {
		MessageBox(NULL, __T("Blit Failed"), __T("Error"), MB_OK);
	}
}
